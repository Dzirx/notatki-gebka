...
class Notatka(Base):
    ...
    # Relacje - POPRAWKA: zmiana konfiguracji cascade
    ...
    przypomnienia = relationship(
        ...
        passive_deletes=True  # DODANE: pozwala bazie danych obsłużyć usuwanie
    )
...
class Przypomnienie(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    # POPRAWKA: dodanie ondelete="CASCADE" do ForeignKey
...
class KosztorysTowar(Base):
    ...
    cena = Column(DECIMAL(10, 2), nullable=False)  # POPRAWIONE: było cena_jednostkowa
    # USUNIĘTO: wartosc - nie ma tej kolumny w schemacie bazy  
    created_at = Column(DateTime, server_default=func.now())
...
class KosztorysUsluga(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    kosztorys_id = Column(Integer, ForeignKey("kosztorysy.id"))
    uslugi_id = Column(Integer, ForeignKey("uslugi.id"))  # POPRAWIONE: było usluga_id
    ilosc = Column(DECIMAL(10, 2), nullable=False)
    cena = Column(DECIMAL(10, 2), nullable=False)  # POPRAWIONE: było cena_jednostkowa
    # USUNIĘTO: wartosc - nie ma tej kolumny w schemacie bazy    
    created_at = Column(DateTime, server_default=func.now())
